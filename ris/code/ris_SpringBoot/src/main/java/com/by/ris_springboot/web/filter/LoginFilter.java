...
public class LoginFilter implements Filter {
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
        ...
        // 跳过静态资源
        if (path.startsWith("/jquery/") || path.contains("/css/") || path.contains("/js/") || path.contains("/image/")) {
            chain.doFilter(req, resp);
        }
        //不应该被拦截的资源，自动放行请求
        if("/login.jsp".equals(path) || "/settings/user/login.do".equals(path) || "/settings/user/faceLogin.do".equals(path)||"/workbench/Report/get_python_message.do".equals(path)){
